    ...
    * Reading and writing are supported.
    ...
    def read(self, filename, assume_volumetric=False):
        ...
        with mrcfile.open(filename) as mrc:
            ...
            if not np.allclose(
                [h.cellb.alpha, h.cellb.beta, h.cellb.gamma], [90, 90, 90]
            ):
                raise ValueError(
                    "Only orthorhombic unitcells are currently "
                    "supported, not "
            ...
            self.delta = np.diag(
                np.array([mrc.voxel_size.x, mrc.voxel_size.y, mrc.voxel_size.z])
            )
    ...
    def edges(self):
        ...
        return [
            self.delta[d, d] * np.arange(self.shape[d] + 1)
            + self.origin[d]
            - 0.5 * self.delta[d, d]

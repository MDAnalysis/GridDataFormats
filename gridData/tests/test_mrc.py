...
def _test_ccp4(g):
    ...
    assert_allclose(g.delta, [3.0 / 4, 0.5, 2.0 / 3])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "cella",
            np.rec.array(
                (45.8, 45.8, 89.65), dtype=[("x", "<f4"), ("y", "<f4"), ("z", "<f4")]
            ),
        ),
        (
            "cellb",
        ...
        (
            "origin",
            np.rec.array(
                (0.0, 0.0, 0.0), dtype=[("x", "<f4"), ("y", "<f4"), ("z", "<f4")]
            ),
        ),
        ("map", b"MAP "),
...
def test_delta(ccp4data):
    assert_allclose(
        ccp4data.delta,
        np.array(
            [[0.5452381, 0.0, 0.0], [0.0, 0.5452381, 0.0], [0.0, 0.0, 0.5603125]],
            dtype=np.float32,
        ),
    )
